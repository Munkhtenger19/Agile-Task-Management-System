...
export async function generateBoardTasks(boardTitle: string, description?: string) {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash-lite" });
    
    const prompt = `You are a project manager. Given the project title "${boardTitle}"${description ? ` and description "${description}"` : ""}, generate a list of 5-8 essential tasks to complete this project.
